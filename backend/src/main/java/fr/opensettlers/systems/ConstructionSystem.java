...
public class ConstructionSystem implements ISystem {
    ...
    private Worker spawnWorkerFromWarehouse(GameState state, Building targetBuilding, WorkerType type) {
        StorageBuilding warehouse = findNearestSpawningWarehouse(state, targetBuilding.getPosition());
        if (warehouse != null) {
    ...
    /**
     * Resolves the nearest warehouse able to train a worker (holding at least
     * one neutral settler and one tool).
     *
     * @param state the current game state
     * @param pos   the coordinates from which to measure distance
     * @return the nearest eligible StorageBuilding, or null if none
     */
    private StorageBuilding findNearestSpawningWarehouse(GameState state, Coordinates pos) {
        StorageBuilding nearest = null;
        double minDist = Double.MAX_VALUE;
        for (Building b : state.getBuildings()) {
            if (b instanceof StorageBuilding sb && !sb.isDestroyed() && sb.canSpawnWorker()) {
