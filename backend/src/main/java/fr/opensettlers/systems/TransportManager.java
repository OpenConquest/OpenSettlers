...
public class TransportManager {
     ...
     * On a donkey-assisted level-2 road, the carrier walks several steps per tick.
     */
    public void processCarrier(Carrier carrier, Road road) {
        switch (carrier.getState()) {
            ...
            case WALKING_TO_PICKUP -> {
                for (int step = 0; step < carrierSpeed(road); step++) {
    ...
    /**
     * Returns the number of steps the carrier walks per tick on the given road.
     * A donkey-assisted main road doubles the base speed.
     *
     * @param road the road being walked
     * @return steps per tick
     */
    private int carrierSpeed(Road road) {
        return road.isDonkeyAssisted() ? GameConfig.DONKEY_ROAD_SPEED : 1;
    }
    ...
    private void handleDelivery(Carrier carrier, Road road) {
        ...
        if (resource != null) {
            ...
            road.setTransportCount(road.getTransportCount() + 1);
        }
    }
